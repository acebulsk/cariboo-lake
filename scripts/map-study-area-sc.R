...
cl_watershed <- read_sf('data/gis/cshs-gis-working/catchment_100_th/catchment_boundary.shp') |> 
  st_transform(st_crs(cl_waterbodies)) |> 
  st_crop(bb)
...
cl_rivers <- st_read('data/gis/watercourse_250k_clipped.shp') |> 
  st_transform(st_crs(cl_waterbodies)) |> 
  st_intersection(cl_watershed)

cl_glc <- st_read('data/gis/glims/glims_cl_clip_update.shp')
...
water_col <- 'steelblue1'
glacier_col <- 'white'

sc_map <- tm_shape(cl_dem) +
  tm_raster(palette=sequential_hcl(5, palette = "Terrain2"), 
            title = 'Elevation (m)', 
            breaks = seq(500, 3000, 500))+
  tm_graticules(n.x = 3,
                n.y = 3) +
  # tm_shape(cl_rivers) +
  # tm_lines(col = water_col) +
  tm_shape(cl_waterbodies) +
  tm_fill(col = water_col) +
  tm_borders(col = water_col,
             # lty = "dotted",
             lwd = 0.5) +
...
tmap::tmap_save(sc_map, filename = 'figs/maps/cl_small_scale_updt_basin.png', width = 7, height = 7, insets_tm = inset, insets_vp = vp)
