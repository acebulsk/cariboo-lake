...
v1_raw <- read.csv("data/long_cores/226_VarveThickness_5_varveCalc_RAW.csv") 
v2_raw <- read.csv("data/long_cores/224_varveCounting_4_RAW.csv")
v1_min_varve_depth <- v1_raw$Cumulative.Depth..cm.[2]*10 # mm had to hard code this because we define the distrubed top flag layer
v2_min_varve_depth <- v2_raw$Cumulative.Depth..mm.[2]
...
v1_n_sd_fltr_turb <- 6 # this gets us exactly to the number of layers identified as turbidites using grain size analysis
v2_n_sd_fltr_turb <- 2 # this gets us CLOSE to the number of layers identified as a turbidite.. refilter lower down to get to 7 varves identified as turbidites based on close alignment with the core depth. one additional varev is left identified on visual identification alone. 
...
ek <- df.ek %>%  
  ...
  dplyr::ungroup() %>% 
  mutate(lyr_flag = case_when(
    layer_thickness_mm > 3*sd ~ T,
    TRUE ~ F
  )
  ) |> 
  select(-c(img_J_numb:Year, 
            cumul_depth_mm)) # cumul_depth_mm is incorrect
...
ek13 <- ek %>%
  filter(core_num == "E13", 
         lyr_flag == F) |> 
  mutate(core_depth = cumsum(layer_thickness_mm))

ggplot(ek13, aes(year_bp, core_depth)) +
  geom_point() +
  scale_y_continuous(trans = 'reverse') +
  geom_smooth(method = 'lm', se = F, fullrange=TRUE, linetype = "dashed", size= 0.5) 
  

ek13_lm <- lm(core_depth ~ 0 + year_bp, data = ek13)
...
ggplot(ek_v1, aes(year_bp, core_depth, colour = core_num)) +
  geom_point() +
  scale_y_continuous(trans = 'reverse') 
...
ggplot(ek_v2, aes(year_bp, core_depth/10, colour = core_num)) +
  geom_point() +
  scale_y_continuous(trans = 'reverse') +
  geom_smooth(method = 'lm', se = F, fullrange=TRUE, linetype = "dashed", size= 0.5) +
  # xlab("Couplet Number") +
...
ggplot(ek_all, aes(year_bp, core_depth, colour = core_num)) +
  geom_point() +
  scale_y_continuous(trans = 'reverse') +
  geom_smooth(method = 'lm', se = F, fullrange=TRUE, linetype = "dashed", size= 0.5) +
  # xlab("Couplet Number") +
...
ek_v2_cln <- ek_v2 %>% 
  select(year_bp, depth = core_depth, `Core ID` = core_num) %>% 
...
# clean up EK13 dataframe to add into V1 and V2 records as per CJES reviewer suggestion

ek13_clean <- ek |>
  filter(core_num == 'E13') |> 
  rename(lyr_mm = layer_thickness_mm) |> 
  mutate(
    sd_flag = lyr_flag,
    turbidite_flag = F,
    disturbed_flag = lyr_flag,
    core = 'E13', notes = NA) |> 
  select(lyr_mm, notes, sd_flag, turbidite_flag, disturbed_flag, lyr_flag, core)

#### V1 ####

v1 <- v1_raw %>% 
  select(lyr_mm = Couplet.Thickness..cm., # original layer thicknesses by alex 2017
         notes = Notes) %>% 
  mutate(lyr_mm = lyr_mm * 10) %>% 
...
v1_turb_flag_level <- v1.mean + (v1_n_sd_fltr_turb*v1.sd) 

sd_flag_vect <- v1$lyr_mm > v1_turb_flag_level

v1$sd_flag <- sd_flag_vect

manual_flags <- v1$notes %in% c('disturbed', 'Disturbed')

# we need to define the layers considered to be turbidites 

v1$turbidite_flag <- !manual_flags & sd_flag_vect

v1$turbidite_flag[751] <- T

# this is used to only add years for the disturbed layers and not the turbs

v1$disturbed_flag <- manual_flags | sd_flag_vect & !v1$turbidite_flag
...
v1$lyr_mm[1] <- v1$lyr_mm[1] - sum(ek13_clean$lyr_mm) # since we are adding e13 on top of V1 the top disturbed layer is now smaller to calculate tihs we take the length of core v1's disturbed top and subtract core E13 total length to get the remaining disturbed section of V1 that will be interpolated using a moving average 

v1$core <- "V1"

# add in EK13 to top of core v1 and then we will filter and cumulate 

ek13_v1_clean <- rbind(ek13_clean |> mutate(core_sub_id = 'E13-V1'), v1|> mutate(core_sub_id = core))
...
v1.sd.fltr <- sd(v1_clean_cumsum$lyr_mm_cln, na.rm = T)
v1.mean.fltr <- mean(v1_clean_cumsum$lyr_mm_cln, na.rm = T)
...
v1_clean_cumsum$lyr_mm_stdep <- (v1_clean_cumsum$lyr_mm - v1.mean.fltr)/v1.sd.fltr

v1_turbidite <- v1_clean_cumsum %>% 
  filter(turbidite_flag == T) 

v1_mod <- data.frame(
  year_bp = seq(from = min(v1_clean_cumsum$year_bp), to = max(v1_clean_cumsum$year_bp), by = 1)
) %>% 
  left_join(v1_clean_cumsum) |> 
  group_by(core) |> 
  mutate(lyr_num = if_else(is.na(core), NA, row_number()))

saveRDS(v1_mod, 'data/long_cores/v1_226_w_e13_processed.rds')

e13_mod <- v1_mod |> filter(core =='E13')
saveRDS(e13_mod, 'data/long_cores/e13_processed.rds')

# see how the new interpolated years compares
v1_mod %>%
  ggplot(aes(core_depth, year_bp, colour = core)) +
  geom_point() +
  scale_y_continuous(trans = 'reverse') +
...
v2 <- v2_raw %>% 
  select(lyr_mm = Couplet.Thickness..mm., # original layer thicknesses by alex 2017
         notes = Notes) 
...
# v2.fvl <- v2.mean + (v2_n_sd_fltr_turb*v2.sd) # rm couplets with thicknesses greater than 3 std above the mean

v2_turb_flag_level <- 3.9 # based off visual inspection

sd_flag_vect <- v2$lyr_mm > v2_turb_flag_level

disturbed_flag_vect <- v2$notes %in% fltr_notes

# we need to define the layers considered to be turbidites 

turbidite_flag_vect <- v2$notes %in% c('flood', 'Flood', '')

v2$turbidite_flag <- turbidite_flag_vect & sd_flag_vect

# change some flags based on varve thickness anomalies here that did not correspond to grain size anomalies using varve thickness to ID which ones do not correspond to those ID'd as turbidites through the grain size analysis see 04-turbidites.R for the grain size dataframe
v2_varve_thickness_not_turbidite <- c(4.039, # this corresponds to a depth of roughly 278.4266 cm likely just a thick varve not turbidite
                                      3.945) # this corresponds to a depth of roughly 261.0240 cm likely just a thick varve not turbidite
                                     # 5.132) # keeping this one as... this corresponds to a depth of roughly 180.0107 cm can also see this one on the image 224B+_69-120.5 at a img ruler depth of 78-79 cm was visually identified as a flood


stopifnot(length(v2_varve_thickness_not_turbidite) == length(v2$turbidite_flag[v2$lyr_mm %in% v2_varve_thickness_not_turbidite])) # make sure not grabbing more layers than expected
v2$turbidite_flag[v2$lyr_mm %in% v2_varve_thickness_not_turbidite] <- F
v2$disturbed_flag[v2$lyr_mm %in% v2_varve_thickness_not_turbidite] <- T # since these layers do not match up with our grain size stats we will set to disturbed instead


# this is used to only add years for the disturbed layers and not the turbs

v2$disturbed_flag <- disturbed_flag_vect | sd_flag_vect & !v2$turbidite_flag

v2$lyr_flag <- v2$disturbed_flag | v2$turbidite_flag

v2$sd_flag <- sd_flag_vect
...
v2$lyr_mm[1] <- v2$lyr_mm[1] - sum(ek13_clean$lyr_mm) # since we are adding e13 on top of v2 the top disturbed layer is now smaller to calculate tihs we take the length of core v2's disturbed top and subtract core E13 total length to get the remaining disturbed section of v2 that will be interpolated using a moving average 

v2$core <- "V2"

# add in EK13 to top of core v1 and then we will filter and cumulate 

ek13_v2_clean <- rbind(ek13_clean |> mutate(core_sub_id = 'E13-V2'), v2 |> select(names(v1)) |> mutate(core_sub_id = core))

# apply above stats to V2, essentially treats every varve as 1 yr, unless flaged, 
# then take the moving average (100 yr) to calculate the time elapsed for the flagged layer.
v2_clean_cumsum <- ek13_v2_clean %>% 
  mutate(
    core_depth = cumsum(lyr_mm), # same as cumsum_excel but recalc for completeness 
    # need this so we dont take into account floods / disturbed when doing the moving avg
    lyr_mm_cln = case_when(
...
# reduce ekman record so there is no overlap
v2_min_varve_yr_bp <- min(v2_clean_cumsum$year_bp[v2_clean_cumsum$core == 'V2'])
v2_clean_cumsum <- v2_clean_cumsum |> 
  filter(!(core == 'E13' & year_bp >= v2_min_varve_yr_bp))

v2.sd.fltr <- sd(v2_clean_cumsum$lyr_mm_cln, na.rm = T)
v2.mean.fltr <- mean(v2_clean_cumsum$lyr_mm_cln, na.rm = T)
...
v2_clean_cumsum$lyr_mm_stdep <- (v2_clean_cumsum$lyr_mm - v2.mean.fltr)/v2.sd.fltr

v2_clean_cumsum <- v2_clean_cumsum |> 
  select(names(v1_clean_cumsum))

v2_turbidite <- v2_clean_cumsum %>% 
  filter(turbidite_flag == T) 
...
v1_top_sed_rate <- mean(v1_clean_cumsum$avg_sed_rate[4]) # this is pretty close to the varve we are after it was just lumped into a large disturbed measurement
...
v2_mod <- data.frame(
  year_bp = seq(from = min(v2_clean_cumsum$year_bp), to = max(v2_clean_cumsum$year_bp), by = 1)
) %>% 
  left_join(v2_clean_cumsum) |> 
  group_by(core) |> 
  mutate(lyr_num = if_else(is.na(core), NA, row_number()))

saveRDS(v2_mod, 'data/long_cores/v2_224_w_e13_processed.rds')
...
long_core_cln <- comb |> 
  filter(core != 'E13') |> 
  rbind(e13_mod) |> 
  select(year = year_bp, depth = core_depth_no_turb, core, date_type, lyr_num) %>% 
  group_by(core) |> 
  mutate(depth = depth / 10,
         ams_cal_se = NA,
...
main.plot <- ggplot(data = all_df, aes(year, depth, colour = core)) +
  ...
  ylab("Core Depth (cm)") +
  theme_bw() +
  labs(shape = '', colour = 'Core:', linetype = 'Age-depth Model:') +
  # scale_color_manual(values = viridis::tur(4)) +
  scale_shape_manual(values = c(17, 3)) +
  # theme(legend.title = element_blank()) + 
  guides(color = guide_legend(override.aes = list(title = 'Core:', shape = 15, linetype = 0, size=5, fill = 'white')))
...
inset.plot <- ggplot(data = all_df, aes(year, depth, colour = core)) +
  ...
  geom_ribbon(aes(xmin = year-varve_se,
              ...
              alpha = 0.25, linetype = 'blank') +
  geom_point(data = turb_c14_df,
             aes(year, depth, shape = point_type),  size = 6) +
  # geom_errorbarh(data = subset(all_df, ams_cal_se != 0), aes(xmin=year-ams_cal_se, xmax=year+ams_cal_se, height = 25)) +
  xlab("Estimated Age (cal yr BP)") +
  ylab("Core Depth (cm)") +
  theme_bw() +
  # xlim(c(NA, 150)) +
  scale_y_continuous(trans = 'reverse') +
  coord_cartesian(xlim=c(-65, 40), ylim=c(20, 0)) +
  # scale_color_manual(values = viridis::viridis(4)) +
  scale_shape_manual(values = c(17, 3))  +
  labs( shape = '', colour = '') +
  theme(legend.position = 'none')+
  guides(color = guide_legend(override.aes = list(shape = 16, linetype = 0, size=5, fill = 'white')))


inset.plot

inset.smaller <- ggplot(data = long_core_cln, aes(year, depth, colour = core)) +
  geom_point()  +
  geom_ribbon(aes(xmin = year-varve_se,
                  xmax = year+varve_se),
              alpha = 0.25, linetype = 'blank') +
  ...
  # xlim(c(NA, 150)) +
  scale_y_continuous(trans = 'reverse') +
  coord_cartesian(xlim=c(-27, -16), ylim=c(9, 6.5)) +
  # scale_color_manual(values = viridis::viridis(4)) +
  scale_shape_manual(values = c(17, 3))  +
  labs( shape = '', colour = '') +
  theme(legend.position = 'none',
...
plot.with.inset <-
  ...
  cowplot::draw_plot(inset.smaller, 
                     x = .58,
                     y = .57, 
                     width = .4, 
                     height = .4)
...
plot.with.inset
cowplot::plot_grid(plot.with.inset, 
                   main.plot, 
                   labels = c('A', 'B'),
                   rows = 2)


ggsave('journal-submission/markdown/figs/longcore_cumulative_depth_vs_estimated_year_w_ams_and_varve.png', width = 6, height = 7)
