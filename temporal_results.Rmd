...
Laminae couplets with thicknesses greater than 3 standard deviations from the mean were classified as event-based turbidite beds. These layers were also obsereved to have well defined beds and were well graded. Table 1, shows the eight turbidites observed in V1 and nine turbidites observed in V2, and their corresponding laminae thickness and estimated year using the 14C-derived accumulation rates. Figure 5, shows the timing of the turbidite beds are not correlated between V1 and V2. It is likely the turbidite beds are formed by more isolated events proximal to the core locations such as adjacent hillslope failures, subaqueous side-wall slumps and/or very local tributary stream floods. For the purpose of establishing an accumulation rate chronology based on the varve only couplets, the turbidite beds were removed from the varve counting chronology.

```{r turbTbl, echo=F, fig.cap='Timing and thickness of turbidite laminae (Lam.) for V1 and V2. Est. Year (CE) is the estimated year using linear interpolation from the AMS radiocarbon dates.'}
...
```{r turbScatter, echo = F, fig.cap = "Standardized departures for grain size and varve thickness for turbidites identified in V1 and V2. Year (CE) is the estimated year using linear interpolation from the AMS radiocarbon dates."}
...
Event-based layers, shown in Figure \@ref(fig:turbScatter) were removed prior to subsequent trend analyses of varve thickness, grain size, and percent organics. The event-based layers were identified if the thickness and grain size were at least 3 standard deviations from the mean, with mean standardized departures of 13.9 and 10.0 respectively. The loss on ignition (LOI) for the event-based layers do not show as strong of a departure from the mean with an average standardized departure of -1.8. Still, most of the event-based layers had below average LOI aside from two that were above average which occurred around 250 CE and 750 CE. 

The timing of the even-based layers between V1 and V2 coincide for four of the seven layers. This could be due to some events occuring from strong precipitation or rain-on-snow events deliverying high sediment-yield to the lake wide leading to some coincident event layers at both cores. While some event layers may be more locally derived turbidite plumes from more local hillslope failures. The layers hypothesized to be from lake-wide events occur around 1250, 1600, and 1950 CE.

The composition of sediment grains within the event-based layers were all characterized by a single mode with less than 0.01% clay, over 98% silt and less than 1% sand. The average D50 particle size of the event layers is 9.7 µm and 16.2 µm for V1 and V2 respectively and for couplet sediment is 7.6 µm and 6.4 µm for V1 and V2 respectively. The particle size distribution for the couplet sediments is characterized by a bi-modal distribution with an average composition of 13% clay, 86% silt, and 0.8% sand sized particles at V1 and 81% silt, 18% clay, and 0.5% sand sized at V2. 
...
```{r particle-b, echo=FALSE, fig.width = 8, fig.height = 4, fig.cap = " OPTION B - Standardized departure from the mean D50 grain size for cores V1 and V2. The black points represent D50 particle size at 5 - 10 cm intervals and the gray line is the 3 sample (~125 year) moving average. The top axes, labelelled Year (CE), are calculated as per Figure X. The black X's on the bottom graph of V1 and V2 denote the AMS radiocarbon age (± dating error) and depth of the respective sample."}
