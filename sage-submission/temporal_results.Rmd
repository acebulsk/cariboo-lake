...
ams_meta |> 
  ...
  kableExtra::add_footnote('Note that the 2-sigma and median ages shown here are based on IntCal13.',notation = 'symbol')
```
...
```{r particle, echo=FALSE, fig.width = 8, fig.height = 6, fig.cap = "Standardized departure from the mean D~50~ grain size for cores V1 and V2. The black points represent D~50~ grain size at 5 - 10 cm intervals and the gray line is the 3 sample (~125 year) moving average. The top axes, labeled Year (CE), was estimated using linear interpolation from the median calibrated $^{14}$C age. The black X's on the bottom graph of V1 and V2 denote the AMS radiocarbon age ($\pm$ dating error) and depth of the respective sample.\\label{particle}"}
...
```{r loi, echo=FALSE, fig.width = 8, fig.height = 6, fig.cap = "Standardized departure from the mean percent organic matter (OM) for cores V1 and V2. The black points represent percent OM at 2.5 - 5 cm intervals and the gray line is the 3 sample (~75 year) moving average. The top axes, labelelled Year (CE), was estimated using linear interpolation from the median calibrated $^{14}$C age. The black X's on the bottom graph of V1 and V2 denote the AMS radiocarbon age ($\pm$ dating error) and depth of the respective sample.\\label{loi}"}
